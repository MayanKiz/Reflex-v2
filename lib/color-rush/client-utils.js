...
export function makeBoard() {
  ...
  const commands = ['FIND', 'FIND ANY', 'MATCH'];
  const command = `${commands[Math.floor(Math.random() * commands.length)]} ${target.name.toUpperCase()}`;
  return { target, board, displayColor, command };
}
